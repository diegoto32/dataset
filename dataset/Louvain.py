    ...
    with open(path) as text:
        for line in text:
            ...
            if len(vertices) >= 3:
                v_i = int(vertices[0])
                v_j = int(vertices[1])
            #w = 1# 数据集有权重的话则读取数据集中的权重
                w = int(vertices[2])
            #Deg = float(vertices[2])
                G[v_i][v_j] = w
                G[v_j][v_i] = w
            else:
...
class Louvain:
    def __init__(self, G):
        ...
        degrees = {vid: sum(self._G[vid].values()) for vid in self._G.keys()}
        avg_degree = statistics.mean(degrees.values())
        std_degree = statistics.stdev(degrees.values())
        #seeds = [vid for vid, degree in degrees.items() if avg_degree +1.5* std_degree < degree < avg_degree + 2.5*std_degree ]
        seeds = [vid for vid, degree in degrees.items() if 5 < degree < 7]
        self._S = set(seeds)

        print("Seed Nodes (S):", self._S)
...
def showCommunity(G, partition, pos):
    ...
    for index, item in enumerate(partition):
        color = colors[index % len(colors)]  # 使用模运算来循环使用颜色列表中的颜色
        shape = shapes[index % len(shapes)]  # 使用模运算来循环使用形状列表中的形状
        nx.draw_networkx_nodes(G, pos, nodelist=item,
                               node_color=color,
                               node_shape=shape,
                               node_size=10,
    ...
    for index, edgelist in enumerate(edges.values()):
        ...
        if index < len(partition):
            edge_color = colors[index % len(colors)]# 使用模运算来循环使用颜色列表中的颜色
            ...
            nx.draw_networkx_edges(G, pos,
                                   edgelist=edgelist,
                                   width=3, alpha=0.8, edge_color=edge_color)
...
if __name__ == '__main__':
    ...
    obj = Graph()
    G1 = obj.createGraph("data/football.txt")
